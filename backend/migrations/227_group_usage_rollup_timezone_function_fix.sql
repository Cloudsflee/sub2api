-- Use the rollup state's configured timezone when invalidating daily watermarks.
-- Migration 223 used the database session timezone, which can disagree with
-- timezone_name (and rewound an Asia/Shanghai watermark around UTC midnight).
...
BEGIN
    SELECT closed_before,
           COALESCE(NULLIF(timezone_name, ''), current_setting('TimeZone'))
