...
// otherwise ChatGPT manifests are normally proxied verbatim. When the Codex
// image bridge is enabled, affected models advertise full Responses so hosted
// tools can be injected. Custom API key manifests receive provider-compatibility
// normalization plus short-lived caching.
// Pinned discovery takes precedence over local account model mappings. When
// disabled, groups with explicit mappings are generated locally; custom API key
// manifests receive provider-compatibility normalization plus short-lived caching.
