...
func openAIStreamFailedEventRetryableOnSameAccount(account *Account, payload []byte, message string) bool {
	...
	// 没有明确降载错误码时，泛化的 model-at-capacity/servers-overloaded
	// 文案仍按账号切换和冷却处理。
	if isOpenAIModelCapacityError(message, payload) {
		return false
	}
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverError(
	...
	// 显式的请求级容量降载码优先于泛化的 overloaded 文案；否则会被
	// 误归类为账号级容量故障，跳过同账号重试并触发错误冷却。
	if isOpenAIModelCapacityError(message, payload) &&
		!isOpenAIUpstreamCapacityShedEvent(payload) {
		failoverErr.RetryableOnSameAccount = false
		failoverErr.RequestScopedTransient = false
		failoverErr.Scope = GatewayFailureScopeAccount
		failoverErr.Reason = openAIModelCapacityFailureReason
