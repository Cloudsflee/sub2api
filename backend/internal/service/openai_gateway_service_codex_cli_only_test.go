...
func TestOpenAIGatewayService_Forward_ModelCapacityErrorTriggersAccountFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestOpenAIGatewayService_Forward_ServerOverloadedMessageRemainsRequestScoped(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"application/json"},
				"x-request-id": []string{"rid-overloaded-400"},
			},
			Body: io.NopCloser(strings.NewReader(`{"error":{"message":"Our servers are currently overloaded. Please try again later.","type":"invalid_request_error"}}`)),
		},
	}
	svc := &OpenAIGatewayService{
		cfg: &config.Config{
