...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if !openAICodexAccountCanServeRequestedModel(account, req.RequestedModel) {
		return false, "plan_not_entitled"
	}
...
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleResult(account *Account, model string, success bool, firstTokenMs *int, observedErr ...error) bool {
	...
	if success {
		...
		s.clearOpenAIModelCapacityState(accountID, normalizeOpenAIAccountModelTransientModel(model))
	} else if len(observedErr) > 0 {
		var failoverErr *UpstreamFailoverError
		if errors.As(observedErr[0], &failoverErr) && failoverErr.IsOpenAIModelCapacityError() {
			s.recordOpenAIModelCapacityFailure(accountID, normalizeOpenAIAccountModelTransientModel(model), time.Now())
		}
	}
