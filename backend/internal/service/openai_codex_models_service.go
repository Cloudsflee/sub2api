...
func (s *OpenAIGatewayService) fetchCodexModelsManifestUpstream(ctx context.Context, request openAIModelsRequest, ifNoneMatch string) (*OpenAIModelsResponse, error) {
	...
	if adjustForFullResponses {
		body, err = adjustCodexModelsManifestForFullResponses(body, request.credentialAccount)
		if err != nil {
			return nil, &codexModelsManifestUpstreamError{
				err: infraerrors.Newf(
					http.StatusBadGateway,
...
// Responses Lite whenever the downstream route needs full Responses support.
// Return the original body when no targeted true value is present so upstream
// ETags and pass-through behavior remain unchanged.
func adjustCodexModelsManifestForFullResponses(body []byte, account *Account) ([]byte, error) {
	var envelope map[string]json.RawMessage
	if err := json.Unmarshal(body, &envelope); err != nil {
...
// adjustAPIKeyCodexModelsManifest keeps the API-key call sites explicit while
// sharing the same full-Responses compatibility transform with the OAuth image bridge.
func adjustAPIKeyCodexModelsManifest(body []byte, account *Account) ([]byte, error) {
	return adjustCodexModelsManifestForFullResponses(body, account)
}
...
func buildOpenAIModelsCacheKey(request openAIModelsRequest) string {
	hasher := sha256.New()
	_, _ = fmt.Fprintf(hasher, "%d\n%d\n%t\n%t\n%s\n%s\n", request.accountID, request.credentialAccountID, request.forceFullResponses, request.standardModelsList, request.proxyURL, request.url)
