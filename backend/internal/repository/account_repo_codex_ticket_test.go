...
func TestLockAndMergeAccountExtraPreservesLatestCodexTicket(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT.*FOR NO KEY UPDATE`).
		...
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "current_wake_scope", "wake_identity_unchanged", "current_extra", "enabled", "rate_sync_enabled", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "current_extra_json", "opencode_group_unchanged", "opencode_auto", "opencode_snapshot"}).
			AddRow(true, false, true, true, true, []byte(`{}`), nil, nil, nil, nil, nil, nil, []byte(`{"codex_turn_ticket:model":{"state":"latest-database-ticket"},"old_admin_setting":true}`), false, nil, nil))
...
func TestLockAndMergeAccountExtraDegradesOnUnparsableExtra(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT.*FOR NO KEY UPDATE`).
		WithArgs(int64(41), service.PlatformOpenAI, service.AccountTypeOAuth, `{"access_token":"test"}`, nil, service.QuotaDimensionGlobal, true).
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "current_wake_scope", "wake_identity_unchanged", "current_extra", "enabled", "rate_sync_enabled", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "current_extra_json", "opencode_group_unchanged", "opencode_auto", "opencode_snapshot"}).
			AddRow(true, false, true, true, true, []byte(`{}`), nil, nil, nil, nil, nil, nil, []byte(`[1,2,3]`), false, nil, nil))
