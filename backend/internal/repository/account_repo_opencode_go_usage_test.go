...
// lockAndMergeAccountProbeExtra 的 SELECT 同时包含 5h wake 与 OpenCode 受管状态列，
// 供通用 Update 路径做两类状态的原子回填。
func openCodeGoMergeMockColumns() []string {
	return []string{
		"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged",
		"current_wake_scope", "wake_identity_unchanged", "current_extra",
		"enabled", "rate_sync_enabled", "snapshot",
		"ollama_session", "ollama_auto", "ollama_snapshot", "current_extra_json",
...
func TestLockAndMergeAccountProbeExtraPreservesOpenCodeGoManagedState(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				WithArgs(tt.account.ID, tt.account.Platform, tt.account.Type, string(credentials), nil, service.QuotaDimensionGlobal, true).
				WillReturnRows(sqlmock.NewRows(openCodeGoMergeMockColumns()).
					AddRow(false, false, tt.proxyUnchanged, false, false, []byte(`{}`), nil, nil, nil, nil, nil, nil, []byte(`{}`), tt.groupUnchanged, tt.databaseAuto, tt.databaseSnapshot))
