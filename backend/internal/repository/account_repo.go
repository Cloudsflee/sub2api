...
func lockAndMergeAccountProbeExtraWithIdentity(
	...
	var ticketExtra map[string]any
	if len(currentExtraJSON) > 0 {
		if err := json.Unmarshal(currentExtraJSON, &ticketExtra); err != nil {
	...
	extra = service.MergeOpenAICodexTicketExtra(extra, ticketExtra)
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	if len(updates.Extra) > 0 || len(ollamaGroupIdentityChanges) > 0 || ollamaProxyIdentityChanged != "" || openAIWakeIdentityCredentialUpdate || updates.EnsureCodexFingerprintSeed {
