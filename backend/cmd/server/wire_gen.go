...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	codexTicketProxySyncService := service.ProvideCodexTicketProxySyncService(client, settingRepository, accountRepository, proxyRepository, schedulerCache)
	settingService := service.ProvideSettingServiceWithCodexTicketProxySync(settingRepository, groupRepository, proxyRepository, configConfig, codexTicketProxySyncService)
	...
	publicAccountStatusRepository := repository.NewPublicAccountStatusRepository(client)
	publicAccountStatusService := service.NewPublicAccountStatusService(publicAccountStatusRepository, accountUsageService, concurrencyService)
	publicAccountStatusHandler := handler.NewPublicAccountStatusHandler(publicAccountStatusService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, publicAccountStatusHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
